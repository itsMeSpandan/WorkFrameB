...
const PixelBlast = ({
  ...
  // Wire up autoPauseOffscreen with IntersectionObserver
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const needsReinitKeys: Array<keyof typeof cfg> = [
      "antialias",
      "liquid",
      "noiseAmount",
    ];
    ...
    if (mustReinit) {
      ...
      // Optional postprocessing pipeline
          ...
          for (const p of composer.passes) {
            ...
            if (effs) {
              for (const eff of effs) {
                const uTime = eff.uniforms?.get("uTime") as
                  | THREE.Uniform
                  | undefined;
                if (uTime) uTime.value = uniforms.uTime.value;
              }
      ...
      // Update existing uniforms without reinit
      const t = threeRef.current;
      if (!t) return;
      ...
      if (t.liquidEffect) {
        const uMap = t.liquidEffect.uniforms as
          | Map<string, THREE.Uniform>
          | undefined;
        const uStrength = uMap?.get("uStrength");
        if (uStrength) uStrength.value = liquidStrength;
        const uFreq = uMap?.get("uFreq");
